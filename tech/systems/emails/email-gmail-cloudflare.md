...
1. Click **Create**. Cloudflare will send a verification message to your Gmail.

### Verify Forwarding Address

1. In Gmail, open the verification email from Cloudflare and click **Verify**.
1. Return to Cloudflare’s Email Routing tab and confirm the rule status is **Active**.
...
1. Click **Add records** and verify they appear in the **DNS** settings.
...
1. Click your profile icon (top right) and select **Senders, Domains & Dedicated IPs**.
1. Go to the **Domains** tab and click **Add a domain**.
1. Follow the prompts to let Brevo automatically configure DNS in Cloudflare.
1. Wait for DNS propagation and ensure the domain status is **Authenticated**.

### Generate an SMTP Key at Brevo

1. In your Brevo dashboard, click your profile icon and choose **SMTP & API**.
1. Under the **SMTP** tab, click **Generate a new SMTP key**.
1. Copy the SMTP settings and generated key for the next step.
...
1. Complete the verification steps in the confirmation email sent by Gmail.
