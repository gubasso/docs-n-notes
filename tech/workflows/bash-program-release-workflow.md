...
The OBS trigger uses a per-package scoped token (see §5.3.1) — endpoint on `api.opensuse.org`,
literal header `Authorization: Token`, never `Bearer`. `--fail-with-body` surfaces OBS errors so the
Action doesn't silently succeed when OBS rejects the request.
...
- `<tool>-git` — VCS variant tracking `main`, with the standard `pkgver()` function and `-git`
  suffix.
...
source of truth — CI never uploads tarballs to OBS. The CI step in §4 only posts a one-line `curl`
that tells OBS "the tag moved, re-run services" — OBS does the rest.
...
- **[release-please](https://github.com/googleapis/release-please) instead of git-cliff + manual
  ...
  GitHub-locked, Node-based. Fine for SaaS, overkill for a single-maintainer Bash CLI.
